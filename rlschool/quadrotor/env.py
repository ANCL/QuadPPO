...
class Quadrotor(object):
    ...
    def step(self, action):
        ...
        if self.task in ['no_collision', 'hovering_control']:
            ...
            reward = self._get_reward(collision=is_collision,rewardcase = self.rewardcase)
            reset = False
            if is_collision:
                reset = True
                self.ct = 0
    ...
    def _get_reward(self, collision=False, velocity_target=(0.0, 0.0, 0.0), rewardcase=1 ):
        """
        ...
        if self.task == 'no_collision':
            ...
            position_norm = np.linalg.norm(self.simulator.global_position)
            torque_norm = np.linalg.norm( self.simulator.prop_torque)

            # TODO: you may find better ones!
            alpha, beta = 1.0, 1.0
            param1, param2 = 1.0, 1.0
            hovering_range, in_range_r, out_range_r = 0.5, 10, -20

            if rewardcase == 1:
                task_reward -= alpha * velocity_norm + beta * angular_velocity_norm 
            elif rewardcase == 2:
                task_reward -= alpha * velocity_norm + beta * angular_velocity_norm + param1 * position_norm 
            elif rewardcase == 3:
                task_reward -= alpha * velocity_norm + beta * angular_velocity_norm + param1 * position_norm + param2 * torque_norm
